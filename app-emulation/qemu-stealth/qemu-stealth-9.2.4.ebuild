...
if [[ ${QEMU_DOCS_PREBUILT} == 1 ]] ; then
	SRC_URI+=" !doc? ( https://dev.gentoo.org/~${QEMU_DOCS_PREBUILT_DEV}/distfiles/app-emulation/qemu/qemu-${QEMU_DOCS_VERSION}-docs.tar.xz )"
fi

S="${WORKDIR}/${MY_P}"
KEYWORDS="~amd64 ~arm ~arm64 ~loong ~ppc ~ppc64 ~riscv ~x86"

DESCRIPTION="QEMU + Kernel-based Virtual Machine userland tools (Anti-Detection Stealth Build)"
...
PATCHES=(
	"${FILESDIR}"/${PN}-9.2.4-anti-detection.patch
	"${FILESDIR}"/${PN}-9.2.4-nfs-fix.patch
...
This is a STEALTH BUILD of QEMU with anti-detection patches applied.
It includes comprehensive virtualization signature hiding to evade
detection by security software and anti-VM measures."
...
src_install() {
	...
	if ! use doc && [[ ${QEMU_DOCS_PREBUILT} == 1 ]] ; then
		doman "${WORKDIR}"/${PN%-stealth}-${QEMU_DOCS_VERSION}-docs/docs/*.[0-8]
	fi
...
pkg_postinst() {
	...
	elog ""
	elog "IMPORTANT: This is a STEALTH BUILD of QEMU with comprehensive"
	elog "anti-detection patches applied. It includes extensive modifications"
	elog "to hide virtualization signatures from guest systems."
	elog ""
	elog "Features included:"
	elog "  • SMBIOS virtualization signature removal"
	elog "  • CPU identification spoofing"
	elog "  • PCI device ID masquerading"
	elog "  • Input device signature hiding"
	elog "  • ACPI table modifications"
	elog "  • Network device MAC address changes"
	elog "  • USB device vendor/product ID spoofing"
	elog ""
	elog "Use this build responsibly and in accordance with applicable laws."
}
