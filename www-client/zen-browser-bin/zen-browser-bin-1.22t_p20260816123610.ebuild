...
ZEN_TWILIGHT_TAG="zen-browser-${PV}"
SRC_URI="
	amd64? (
		https://github.com/coleleavitt/coleleavitt-overlay/releases/download/${ZEN_TWILIGHT_TAG}/zen.linux-x86_64.tar.xz
		-> ${P}-x86_64.tar.xz
	)
	arm64? (
		https://github.com/coleleavitt/coleleavitt-overlay/releases/download/${ZEN_TWILIGHT_TAG}/zen.linux-aarch64.tar.xz
		-> ${P}-aarch64.tar.xz
	)
"
...
EOF
	...
	# Portage owns the browser installation, so disable only application binary
	# updates. Keep Firefox system-addon and user-extension updates enabled.
