...
multilib_src_configure() {
	# Get the actual LLVM version to ensure pkg-config reports correct version
	# This fixes ABI mismatch issues when LLVM version differs from upstream's hardcoded BASE_LLVM_VERSION
	local llvm_version
	llvm_version=$(llvm-config --version) || die "Failed to get LLVM version"

	local mycmakeargs=(
		-DBASE_LLVM_VERSION="${llvm_version}"
		-DCCACHE_ALLOWED="OFF"
